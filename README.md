# Französische Lernkarten

PHP/MariaDB-Lernkarten-App, abgeleitet aus dem Projekt `buddha(4).zip` und angepasst an die Tabelle `franzoesisch_woerter_und_saetze`.
...
- Filter nach Lektion
- Suche nach ID, deutschem/französischem Wort oder Satz
- Zufallskarten
- Multiple Choice
...
2. Datenbank `franzoesisch_db` anlegen.
3. `database/franzoesisch_woerter_und_saetze.sql` importieren, falls die Tabelle noch nicht existiert.
4. In `config/database.php` Benutzername und Passwort anpassen oder die Umgebungsvariablen `FRANZOESISCH_DB_HOST`, `FRANZOESISCH_DB_USERNAME`, `FRANZOESISCH_DB_PASSWORD`, `FRANZOESISCH_DB_NAME` setzen.
5. PHP 8.1+ mit `mysqli` verwenden.

Die App legt bei Bedarf zusätzlich die Tabelle `französisch_einstellungen` für das Administrationspasswort an.

## Datenmodell

- `wort_de`: deutsches Wort
- `wort_it`: französisches Wort
- `satz_de`: deutscher Beispielsatz
- `satz_it`: französischer Beispielsatz
...
## Admin-Suche
Im Administrationsbereich kann nach Datenbank-ID, deutschem Wort und französischem Wort gesucht werden. Die Suche kann auf ein Feld eingeschränkt oder kombiniert verwendet werden.

## Verben (Erweiterung 18.08.2026)
- Neue Tabelle `französisch_verben` mit ID, Verb, Präsens, Perfekt, Futur, Imperativ und Endung.
...
- Tabellenanlage erfolgt bei Bedarf automatisch über `database/schema.php`; die mitgelieferte SQL-Struktur liegt zusätzlich unter `database/französisch_verben.sql`.
...
- `database/französisch_grammatik.sql` liegt als Fallback-/Referenzstruktur bei. `database/schema.php` legt die Tabelle bei Neuinstallationen ebenfalls an.
...
Die Suche durchsucht Französisch, Deutsch, Präsens, Perfekt, Futur, Imperativ und Endung und unterstützt Teilwörter.
Der PDF-Ausdruck kann wahlweise nach Französisch A–Z oder Deutsch A–Z sortiert werden.
