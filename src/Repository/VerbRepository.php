...
final class VerbRepository
{
    ...
    /**
     * Liefert Verben mit Volltext-/Teilwortsuche über Französisch, Deutsch
    ...
    public function all(string $search = '', string $sort = 'it'): array
    {
        ...
        $select = 'SELECT id, verb_it, verb_de, praesens, perfekt, futur, imperativ, endung FROM französisch_verben';
    ...
    public function add(
        ...
        $stmt = $this->db->prepare(
            'INSERT INTO französisch_verben (verb_it, verb_de, praesens, perfekt, futur, imperativ, endung) VALUES (?, ?, ?, ?, ?, ?, ?)'
        );
        if (!$stmt) {
            return false;
        }
    ...
    public function update(
        ...
        $stmt = $this->db->prepare(
            'UPDATE französisch_verben SET verb_it = ?, verb_de = ?, praesens = ?, perfekt = ?, futur = ?, imperativ = ?, endung = ? WHERE id = ?'
        );
        if (!$stmt) {
            return false;
        }
    ...
    {
        $stmt = $this->db->prepare('DELETE FROM französisch_verben WHERE id = ?');
        if (!$stmt) {
            return false;
        }
