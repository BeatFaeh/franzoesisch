...
final class GrammarRepository
{
    ...
    {
        $result = @$this->db->query('SHOW COLUMNS FROM französisch_grammatik');
        if (!$result) return;
        $columns = [];
    ...
    public function all(string $search = ''): array
    {
        ...
        if ($search === '') {
            $result = $this->db->query("SELECT $select FROM französisch_grammatik ORDER BY $keyword ASC, id ASC");
            return $result ? $result->fetch_all(MYSQLI_ASSOC) : [];
        }
        $searchParts = ["$keyword LIKE ?"];
        if ($content !== null) $searchParts[] = "$content LIKE ?";
        if (ctype_digit($search)) $searchParts[] = 'id = ?';
        $sql = "SELECT $select FROM französisch_grammatik WHERE " . implode(' OR ', $searchParts) . " ORDER BY $keyword ASC, id ASC";
    ...
    public function add(string $stichwort, string $erklaerung, string $pdf): bool
    {
        if ($this->contentColumn !== null) {
            $sql = 'INSERT INTO französisch_grammatik (' . $this->q($this->keywordColumn) . ', ' . $this->q($this->contentColumn) . ', ' . $this->q($this->pdfColumn) . ') VALUES (?, ?, ?)';
            $stmt = $this->db->prepare($sql);
            if (!$stmt) return false;
            $stmt->bind_param('sss', $stichwort, $erklaerung, $pdf);
        } else {
            $sql = 'INSERT INTO französisch_grammatik (' . $this->q($this->keywordColumn) . ', ' . $this->q($this->pdfColumn) . ') VALUES (?, ?)';
            $stmt = $this->db->prepare($sql);
            if (!$stmt) return false;
    ...
    {
        if ($this->contentColumn !== null) {
            $sql = 'UPDATE französisch_grammatik SET ' . $this->q($this->keywordColumn) . ' = ?, ' . $this->q($this->contentColumn) . ' = ?, ' . $this->q($this->pdfColumn) . ' = ? WHERE id = ?';
            $stmt = $this->db->prepare($sql);
            if (!$stmt) return false;
            $stmt->bind_param('sssi', $stichwort, $erklaerung, $pdf, $id);
        } else {
            $sql = 'UPDATE französisch_grammatik SET ' . $this->q($this->keywordColumn) . ' = ?, ' . $this->q($this->pdfColumn) . ' = ? WHERE id = ?';
            $stmt = $this->db->prepare($sql);
            if (!$stmt) return false;
    ...
    {
        $stmt = $this->db->prepare('DELETE FROM französisch_grammatik WHERE id = ?');
        if (!$stmt) return false;
    ...
    public function renamePdfReference(string $oldName, string $newName): void
    {
        ...
        $stmt = $this->db->prepare("UPDATE französisch_grammatik SET $pdf = ? WHERE $pdf = ?");
        if (!$stmt) return;
